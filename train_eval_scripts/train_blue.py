...
from dataset import BlueDataset, FastDataLoader
...
def train(args, models, model_id, model_dir):
  ...
  train_data = BlueDataset(data_file=args.training_file,
                    green_input=args.use_green_input, 
                    green_pred_input=args.use_green_pred,
                    green_file=args.green_training_file,
                    bluegb=args.gb_model, bluegr=args.gr_model, bluer=args.r_model)

  validation_data = BlueDataset(data_file=args.validation_file, 
                          green_input=args.use_green_input, 
                          green_pred_input=args.use_green_pred,
...
def train_epoch(args, train_queue, models, criterion, optimizers, train_loggers, model_pytorch_files, validation_queue, validation_loggers, epoch):
  ...
  for step, (input, target) in enumerate(train_queue):
    green, bayer, img = input 

    green = Variable(green, volatile=True).float().cuda()
    ...
    for i, model in enumerate(models):
      ...
      model_inputs = {"Input(Green)": green, "Input(Bayer)": bayer}
...
if __name__ == "__main__":
  ...
  parser.add_argument('--gr_model', action='store_true')
  parser.add_argument('--gb_model', action='store_true')
  parser.add_argument('--r_model', action='store_true')
  ...
  if args.gr_model:
    model = model_lib.blue_gr_diff_model()
  elif args.gb_model:
    model = model_lib.blue_gb_diff_model()
  elif args.r_model:
    model = model_lib.blue_r_diff_model()
  else:
    assert False, "no blue model specified"
