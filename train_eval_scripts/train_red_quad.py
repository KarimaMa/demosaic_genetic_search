...
from dataset import RedQuadDataset, FastDataLoader
...
def train(args, models, model_id, model_dir):
  ...
  train_data = RedQuadDataset(data_file=args.training_file,
                    green_input=args.use_green_input, 
                    green_pred_input=args.use_green_pred,
                    green_file=args.green_training_file)

  validation_data = RedQuadDataset(data_file=args.validation_file, 
                          green_input=args.use_green_input, 
                          green_pred_input=args.use_green_pred,
...
def train_epoch(args, train_queue, models, criterion, optimizers, train_loggers, model_pytorch_files, validation_queue, validation_loggers, epoch):
  ...
  for step, (input, target) in enumerate(train_queue):
    green_quad, bayer_quad, img = input 
    ...
    for i, model in enumerate(models):
      ...
      model_inputs = {"Input(GreenQuad)": green_quad, "Input(BayerQuad)": bayer_quad}
...
if __name__ == "__main__":
  ...
  model = model_lib.red_quad_diff_model() 
