...
from dataset import FullPredictionProcessedDataset, GreenProcessedQuadDataset
import util
...
def infer(args, test_data, model, model_id):
  ...
  async_loader = AsynchronousLoader(test_data, device, batch_size=args.batchsize, pin_memory=True, \
                                    workers=8, sampler=torch.utils.data.sampler.SequentialSampler(test_indices))
  ...
  with torch.no_grad():
    with profiler.profile(use_cuda=True) as prof:
      with profiler.record_function("model_inference"):
        for step, (input, target) in enumerate(async_loader):
          if args.full_model:
  ...
  prof.export_chrome_trace(f"model-{model_id}-async-loader-trace.json")
