...
class AddOp(nn.Module):
	...
	def eval(self, bayer):
		loperand = self._operands[0].eval(bayer)
		roperand = self._operands[1].eval(bayer)
...
class GreenExtractorOp(nn.Module):
	...
	def forward(self, x, bayer):
		green = green * self.mask + bayer[:,0,...].unsqueeze(1)
...
class SoftmaxOp(nn.Module):
	...
	def eval(self, bayer):
		operand = self._operands[0].eval(bayer)
...
class UpsampleOp(nn.Module):
	...
	def forward(self, x):
		assert self.in_c == x.size()[1], f"Expected channel count of Upsample input to be {self.in_c}"
