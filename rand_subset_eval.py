...
if __name__ == "__main__":
  ...
  for subset in range(args.subsets):
    ...
    for version in range(model_versions):
      infer_logger = util.create_logger(f'{model_id}_inference_logger', logging.INFO, log_format, \
                                        os.path.join(args.save, f'{model_id}_v{version}_inference_log'))

      model, ast = model_manager.load_model(model_id, version)
      full_training_set_loss = infer(args, train_queue, [model], criterion, [infer_logger])[0]

      with open(args.results_file, "a+") as f:
        data_string = f"subset {subset} model version {version} full training loss: {full_training_set_loss} \n"
        f.write(data_string)
