...
def norm_cost(n):
	cost = n * POW_COST + (n-1) * ADDMUL_COST + SQRT_COST
	return cost

def hinterp_cost():
	green_cost = 15 * ADDMUL_COST
	red_cost = 11 * ADDMUL_COST
	blue_cost = 11 * ADDMUL_COST
...
def homogeneity_cost(delta):
	...
	Lcost = SUB_COST + ABSVAL_COST + COMP_COST # compares difference in luminance channel
...
def compute_cost(delta, iterations):
	cost = hinterp_cost()
	print(f"h interp cost {hinterp_cost()}")
	cost += vinterp_cost()
	print(f"v interp cost {vinterp_cost()}")

	cost += 2*lab_conversion_cost() # LAB conversion of h and v images
	print(f"lab_conversion_cost {lab_conversion_cost()}")

	cost += epsilon_cost()
	print(f"epsilon_cost {epsilon_cost()}")

	cost += 2 * homogeneity_cost(delta) # Homogeneity maps of h and v images
	print(f"homogeneity_cost {homogeneity_cost(delta)}")
	...
	cost += artifact_cost(iterations)
	print(f"artifact_cost {artifact_cost(iterations)}")
...
print(f"ahd compute cost with delta={args.delta} iterations={args.iters} {compute_cost(args.delta, args.iters)}")
