...
from global_train import train_model
from dataset import load_data_to_RAM, ids_from_file
...
def load_validation_dataset(args):
  ValInputs, ValLabels = load_data_to_RAM(data_file=args.validation_file)
  val_inputs_shape = ValInputs.shape
  val_labels_shape = ValLabels.shape
  ValInputs = ValInputs.flatten()
  ValLabels = ValLabels.flatten()

  val_inputs_array = mp.Array(ctypes.c_double, ValInputs, lock=False)
  val_labels_array = mp.Array(ctypes.c_double, ValLabels, lock=False)
  return val_inputs_array, val_inputs_shape, val_labels_array, val_labels_shape
...
def load_train_dataset(args):
  ...
  TrainInputs, TrainLabels = load_data_to_RAM(data_filenames=used_filenames)
  TrainInputsShape = TrainInputs.shape
  TrainLabelsShape = TrainLabels.shape
  TrainInputs = TrainInputs.flatten()
  TrainLabels = TrainLabels.flatten()

  train_inputs_array = mp.Array(ctypes.c_double, TrainInputs, lock=False)
  train_labels_array = mp.Array(ctypes.c_double, TrainLabels, lock=False)
  return train_inputs_array, TrainInputsShape, train_labels_array, TrainLabelsShape
...
def run_train(rank, train_args, gpu_id, train_inputs, train_inputs_shape, train_labels, train_labels_shape, \
            val_inputs, val_inputs_shape, val_labels, val_labels_shape, model_id, \
  ...
  try:
    ...
    model_valid_psnrs, model_train_psnrs = train_model(train_args, gpu_id, \
                                                      train_inputs, train_inputs_shape, train_labels, train_labels_shape, \
                                                      val_inputs, val_inputs_shape, val_labels, val_labels_shape, \
...
def init_process(rank, size, fn, train_args, gpu_id, \
                train_inputs, train_inputs_shape, train_labels, train_labels_shape, \
                val_inputs, val_inputs_shape, val_labels, val_labels_shape, model_id, models, model_dir,\
  ...
  fn(rank, train_args, gpu_id, train_inputs, train_inputs_shape, train_labels, train_labels_shape, \
    val_inputs, val_inputs_shape, val_labels, val_labels_shape, model_id, models, model_dir, \
...
class Searcher():
  def __init__(self, args):
    ...
    self.train_inputs_array, self.train_inputs_shape, self.train_labels_array, self.train_labels_shape = load_train_dataset(self.args)
    self.val_inputs_array, self.val_inputs_shape, self.val_labels_array, self.val_labels_shape = load_validation_dataset(self.args)
  ...
  def launch_train_processes(self, mutation_batch_info):
    ...
    for rank in range(size):
      ...
      p = mp.Process(target=init_process, args=(rank, size, run_train, self.args, gpu_id, \
            self.train_inputs_array, self.train_inputs_shape, self.train_labels_array, self.train_labels_shape, \
            self.val_inputs_array, self.val_inputs_shape, self.val_labels_array, self.val_labels_shape, \
            model_id, models, model_dir, train_psnrs, valid_psnrs, log_format, self.debug_logger))
