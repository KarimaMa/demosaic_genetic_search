...
RETRAIN_DATA=$ROOT/retrain_data/FLAGSHIP-CHROMA-INSERT
RETRAIN_LIST=$RETRAIN_DATA/pareto_model_ids.txt
RETRAIN_LOGS=$ROOT/retrain_logs/FLAGSHIP-CHROMA-INSERT
...
python $CODE_LOCAL/sys_run/retrain_one_model.py \
    ...
    --save=$ROOT/results/RETRAINED-FLAGSHIP-CHROMA-INSERT \
    --model_retrain_list=$RETRAIN_LIST \
    --epochs=5 \
    --learning_rate=0.003 \
    --training_file=$DATA_LOCAL/train.txt \
    --validation_file=$DATA_LOCAL/val.txt \
    --green_model_asts=$CODE_LOCAL/PARETO_GREEN_MODELS/flagship-green-pareto-files/pareto_green_asts.txt \
    --green_model_weights=$CODE_LOCAL/PARETO_GREEN_MODELS/flagship-green-pareto-files/pareto_green_weights.txt \
    --full_model
