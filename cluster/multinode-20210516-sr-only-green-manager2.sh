...
python $CODE_LOCAL/multinode_sys_run/manager.py \
    --experiment_name=green-sr-only \
    --training_file=$DATA_LOCAL/train.txt \
    --validation_file=$DATA_LOCAL/val.txt \
    --save=$ROOT/results/MULTINODE-SR-ONLY-GREEN-05-$DATE-RUN$RUN \
    ...
    --tier_size=20 \
    --green_seed_model_files=superres-seed-model-files/sr_only_green_seed_asts.txt \
    --green_seed_model_psnrs=superres-seed-model-files/sr_only_green_seed_psnrs.txt \
    --insertion_bias \
    --late_cdf_gen=9 \
    --max_footprint=30 \
